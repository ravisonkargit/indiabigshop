...
class ProductPolicy extends Component {
    ...
    if (domain_language_code !== "" && domain_language_code !== undefined) {
      await axios
        .post(
          ...
          { security_token: "", plateform_type: "", langCode: domain_language_code,pageid:'13' },
      ...
      await axios
        .post(
          "https://api.beldara.com/common/static_page.php",
          { security_token: "", plateform_type: "", langCode: 'en',pageid:'13' },
          { headers: { "Content-Type": "multipart/form-data" } }
        )
      .then(response => {
          this.setState({
  ...
  render() {
    ...
    return (
      <div>
        <Breadcrumb title={"Product Listing Guidelines"} metaTitle={title} metaDesc={desc1} metaKeyword={keyword}/>
