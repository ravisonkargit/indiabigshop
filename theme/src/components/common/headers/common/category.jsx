    ...
    return (
      <div id="root_category">
        <h4 id="main_cat_to_hover" className="main_cat_to_hover hoverclass">
          Categories{" "}
          <i className="arrow" style={{ fontSize: "15px",margin:"3px 2px" }}></i>
        </h4>
            ...
            .map(([key, value], index) =>
              value.url !== undefined || value.url !== null ? (
                <React.Fragment>
                  {
                                                index == '0'?
                                                <span className={`h5 ${index} mx-1 has-submenu text-capitalize text-center`} style={{paddingTop:'8px'}}>TOP CATEGORIES</span>
                                                : ''
                                            }
                  <li key={key} className="submenu">
                    <a href={`${process.env.PUBLIC_URL}/cat/${value.url.toLowerCase()}.html`} target="_blank">{key}<i className="arrow-down float-right mt-2"></i></a>
                        ...
                        <div className="row m-0">
                          <div className="link-section">
                            <h5 className="text-center">
                              <Link
                                to={`${process.env.PUBLIC_URL}/cat/${value.url.toLowerCase(0)}.html`}
                                target="_blank"
