...
class getStoreCategoryComponent extends Component {
  ...
  componentDidMount = async () => {
    if (this.state.sellerid > 0) {
      await getStoreCategory(this.state.sellerid).then(
        async (res) =>
          await this.setState({
    ...
    if (parseInt(this.state.catid) > 0) {
      ...
      $(".accordion")
        .find("#" + this.state.catid)
        .addClass("cat_label_selected");
    }
  };
  ...
  render() {
    return (
      <div>
        <a
          href={`${this.state.pageUrl}.html`}
          className="showAllCat mouse_pointer"
        >
          Show All{" "}
        </a>
            ...
            this.state.categoryData.length > 0 ? (
              <ul>
                {this.state.categoryData.map((item, index) => (
                  <>
                    {item.cat_id == this.props.cat_id ? (
                      <li key={index} className="catList active-1">
                        <a
                          id={item.cat_id}
                          parent-id={item.parent_id}
                          href={`${this.state.pageUrl}.html/${item.cat_id}`}
                        >
