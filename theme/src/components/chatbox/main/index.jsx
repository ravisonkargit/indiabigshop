...
class Main extends Component {
    constructor() {
        ...
        var addMessage = msg => {
            ...
            if (
                ...
                // console.log(window.localStorage.getItem("log_id"),frmmsg,this.state.toId)
                if (window.localStorage.getItem("log_id") === frmmsg) {
                    ...
                    // console.log(1,217);
                    // this.socket.on('support-msg', async function(msg){
                    //     console.log(msg,216);
                    //     if(msg.result.status == 'success'){
                    //         arr1 = {
                    //             id: NaN,
                    //             author: "orange",
                    //             message: msg.result.error_msg,
                    //             timestamp: new Date().getTime()
                    //         }
                    //        await newArray.push(arr1);
                    //     }
                    // });
    ...
    UNSAFE_componentWillReceiveProps = (nextProps) => {
        ...
        if (nextProps.chatToSeller.chatWithSupplier && this.state.chatWithSupplier) {
            if (localStorage.getItem('sellerid')) {
                if (!this.state.sendToSeller) {
                    // console.log(nextProps, 'receiveprops','UNSAFE_componentWillReceiveProps');
    ...
    componentDidUpdate(prevProps, nextProps) {
        ...
        // console.log(prevProps,'receiveprops','componentDidUpdate');
        if(this.state.step2){
    ...
    chatSubmit = e => {
        ...
        Axios.post(
            ...
            // console.log(response.data,1108);
            if(getCookie('refid') != 'null' && getCookie('refid') !== null && getCookie('refid') != '' && response.data.result.user == 'new_user'){
