...
class StartOrderTest extends Component {
  ...
  componentDidMount = async () => {
    ...
    // axios.post(
    //   `${apiUrl}get_company_bank_details.php`,
    //   {
    //     security_token:"",
    //     plateform_type:"web",
    //     sellerid:ls.get('sellerid'),
    ...
    // ).then(res=>{
    //     if(res.data.statusId == '1'){
    //       this.setState({bank_details:res.data.result[0]})
    //     }
    // }).catch(err=>{
    //   console.error(err);
    // })
  ...
  submitOnlineTransfer = (e) => {
    e.preventDefault();
    $('#error_upload_field').addClass('d-none').html('');
    ...
    var reader = new FileReader();
        ...
        if(regex.test(test_file.toLowerCase())){
          ...
          $('#error_upload_field').removeClass('d-none').html('File type must be jpeg or jpg or png');
  ...
  submitForm = () => {
    ...
    if(seller_file !== undefined){
      ...
      $('#submit_order_receipt').removeAttr('disabled');
      $('#error_upload_field').removeClass('d-none').html('Please Upload File');
    }
  }

  render() {
    // console.log('render',this.state.bank_details);
                      ...
                      <div className="modal-bg addtocart">
                        <div className="modal-body justify-content-center text-center">
                          ...
                          )}
                          {
                            ...
                            ? 
                            <>
                            <span className="d-flex justify-content-center h6">OR</span>
                              <div className="card mt-4">
                              <div className="card-header text-dark py-1">
                                <i className="fa fa-dolly" />
                                NEFT/IMPS TRANSFER
                              ...
                              <div className="card-body py-1">
                                ...
                                <div className="d-flex justify-content-between">
                                  <div>Bank Name</div>
                                  <div>{this.state.bank_details.beneficiary_bank}</div>
                                </div>
                                <div className="d-flex justify-content-between">
                                  <div>Account No</div>
                                  <div>{this.state.bank_details.acc_number}</div>
                                </div>
                                <div className="d-flex justify-content-between">
                                  <div>Bank Branch</div>
                                  <div>{this.state.bank_details.beneficiary_address}</div>
                                </div>
                                <div className="d-flex justify-content-between">
                                  <div>IFSC Code</div>
                                  <div>{this.state.bank_details.bank_swift_code}</div>
                                </div>
                                <input type="file" className="d-none" name="upload_receipt" id="upload_receipt" onChange={this.submitOnlineTransfer} accept="image/x-png,image/gif,image/jpeg" />
                                  <div className="reverse-flow-flex">
                                    <button className="btn" style={custom_button} onClick={this.openfileManager}>Upload File</button>
                                  </div>
                              </div>
                            </div>
                              <div className="" id="preview_image">
                                  <div className="flex-column align-items-end d-flex">
                                    <i id="close_img" onClick={this.clearAll} className="fa fa-close d-none mouse_pointer"></i>
                                  </div>
                                  <img src="" className="d-none" id="img_src"/>
                              </div>
                              <div className="d-flex align-items-end flex-column my-2">
                                  <span className="text-danger d-none" id="error_upload_field">Please Upload File</span>
                                  <button  className="btn btn-solid" id="submit_order_receipt" onClick={this.submitForm}>Submit</button>
                              </div> 
                            </>
                            :""
                          }
