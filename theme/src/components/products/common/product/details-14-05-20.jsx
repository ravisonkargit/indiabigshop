...
class Details extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    if(this.props.item.offer_min_qty !== undefined && this.props.item.offer_min_qty !== null && this.props.item.offer_min_qty !== ''){
  ...
  minusQty = async () => {
    if(this.offerExist(this.props.item.offer_from_date,this.props.item.offer_to_date) && parseInt(this.state.quantity - 1) < parseInt(this.props.item.offer_min_qty)){
      // await this.setState({
      //   qtyerror:true
      // })
    }else{
      if (this.state.quantity > 0) {
        ...
        await this.setState({ quantity: this.state.quantity - 1,qtyerror:false });
      }
      this.CartCalc();
    }
  ...
  plusQty = async () => {
    ...
    if(this.offerExist(this.props.item.offer_from_date,this.props.item.offer_to_date) && parseInt(this.state.quantity + 1) > parseInt(this.props.item.offer_stock)){
      await this.setState({
        qtyerror:true
      })
    }else{
  ...
  changeQty = async (e) => {
    ...
    if(this.offerExist(this.props.item.offer_from_date,this.props.item.offer_to_date) && cqty > parseInt(this.props.item.offer_stock)){
       await this.setState({
          qtyerror:true
        })
    }else{
  ...
  componentDidUpdate = async (nextProps) => {
    // console.log(' componentDidUpdate ',280);
    let search = window.location.search;
    realPath = search.split('beldara.com')[1];
    if (this.props.location.state){
