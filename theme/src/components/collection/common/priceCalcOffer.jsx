...
var start_price, end_price, symbol,mrp_price,price_offer;
class PriceCalcOffer extends Component {
    ...
    getPrice(nextProps){
        // console.log(nextProps,'getPrice','PriceCalcOffer');
        ...
        try{    
            ...
            try{    
                {nextProps.mrp_price === undefined || isNaN(nextProps.mrp_price) || nextProps.mrp_price === null || parseFloat(nextProps.mrp_price) < parseFloat(0) ?
                    mrp_price = 0
                    :mrp_price = nextProps.mrp_price }    
                }catch(e){
                    mrp_price = 0
                }
                ...
                try{    
                    {nextProps.price_offer === undefined || isNaN(nextProps.price_offer) || nextProps.price_offer === null || parseFloat(nextProps.price_offer) < parseFloat(0) ?
                        price_offer = 0
                        :price_offer = nextProps.price_offer }    
                    }catch(e){
                        price_offer = 0
                    }
        ...
        try{
            if(price_offer != 0 && price_offer !== undefined && price_offer !== '' && price_offer !== null){
                // console.log(price_offer,'price_offer',start_price,'before',typeof(price_offer),typeof(start_price));
                start_price  = parseFloat(price_offer);
                // console.log(price_offer,'price_offer',start_price,'after',typeof(price_offer),typeof(start_price));
            }
        ...
        return (
            <React.Fragment>
                <div>
                    ...
                    : <div className="font-weight-bold small"> {parseFloat(start_price) !== 0.00 ? <React.Fragment> {this.prodCost(start_price)} {symbol + ' ' + start_price} </React.Fragment>: 'Ask for price'}</div>
                    :''}
                </div>
                </React.Fragment>
        )
...
export default connect(mapStateToProps,{getAllCurrencyValue})(PriceCalcOffer);
