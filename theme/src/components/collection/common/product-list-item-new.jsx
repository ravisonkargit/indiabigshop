...
class ProductListItemNew extends Component {
      ...
      <div className="product-box bg-light ">
        ...
        <div className="product_info">
          <div>
            ...
            {
              (product.mrp_price != null && product.mrp_price > 0) ?
              <del><div><div class="font-weight-bold small"> {this.props.symbol} {product.mrp_price}</div></div></del>
             : ''
                // <PriceCalc currencyValue={this.props.currencyValue} productCost={this.deadEnd} finalCost={this.finalCost} minqty={product.qty} symbol={product.currency} start_price={product.start_price} end_price={product.end_price} price_us={product.price_us} price_in={product.price_in} /> : <b>Ask For Price</b>
            }
            {
              (product.eachprice > 0 && product.eachprice != null) ?
            <div><div class="font-weight-bold small"> {this.props.symbol} {product.eachprice}</div></div>
             : <b>Ask For Price</b>
                // <PriceCalc currencyValue={this.props.currencyValue} productCost={this.deadEnd} finalCost={this.finalCost} minqty={product.qty} symbol={product.currency} start_price={product.start_price} end_price={product.end_price} price_us={product.price_us} price_in={product.price_in} /> : <b>Ask For Price</b>
            }
            {
              (product.min_qty) ?
                <div className="small">{`${translate('MOQ')} : ${product.min_qty} - ${product.unit}`}</div>
                : <div className="small">{`${translate('MOQ')} : 1 unit`}</div>
            }

            {
              (product.offer_stock != null) ?
          ...
          {product.start_price && parseFloat(product.start_price) > parseFloat(0) ?
            <React.Fragment>
              <div id={`btn_${product.id}`} className=" col-7 col-md-7 col-sm-7 text-left py-2 px-1">
                <div className="mouse_pointer btn btn_Pro  btn-orange" id={product.id} onClick={this.validate.bind(this,'buyBtn', product.id, product.min_qty, (parseFloat(product.start_price) * parseInt(product.min_qty)), this.props.symbol, product.start_price, product.currency, product.sellerid)}>
                  Buy Now
                </div>
              </div>
...
export default withRouter(withTranslate(connect(mapStateToProps)(ProductListItemNew)));
