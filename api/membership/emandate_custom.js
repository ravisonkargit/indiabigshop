...
function getRzrDetail(req) {
  ...
  if (
    req.headers.host === "uat.beldara.com" ||
    req.headers.host === "localhost:5000"
  ) {
    //live purpose
    ...
    if (result[0][0].device_type) {
      ...
      instance.customers
        ...
        .catch(err => {
          console.log("already");
          instance.customers
            .create({ name, email, contact, fail_existing: 0 })
            .then(response => {
...
router.post("/createOrder", (req, res) => {
  ...
  let accType = req.body.accType ? req.body.accType.toLowerCase() : "";
  ...
  if (auth_type === "physical") {
    nachProcedure = {
      nach: {
        form_reference1: "Recurring Payment for " + name,
        form_reference2: "Method Paper NACH " + name,
